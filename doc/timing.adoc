...
[%autowidth, frame=none]
|===
| `time` | integer | Time passed "in-game" since world creation in seconds ("gametime"). Does not increase while the game is paused or not running. Gametime is stored with the world and continuously increases.
|===
...
[%autowidth, frame=none]
|===
| `dtime` | number | Delta (elapsed) time since last step in seconds
|===
...
| `func` | function | Function to be called. Objects with the `__call` metamethod are supported as well.
