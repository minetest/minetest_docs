...
The equation for the length is: stem:[sqrt(x^2 + y^2 + z^2)].
...
Returns a new `vector` which is equal to `v` rotated around `axis` by `angle` radians.

=== `vector.rotate(v, rot) -> vector`
* `v`, `rot`: `vector`

Returns a new `vector` which is equal to `v` rotated by `rot`.
...
WARNING: This function assumes `b` is a number, and will probably return confusing errors about "accessing a `nil` value" if it is not.
