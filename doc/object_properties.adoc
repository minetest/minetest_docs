...
NOTE: This works with players as well but you may expericence https://github.com/minetest/minetest/issues/11783[strange bugs].
...
Whether a model is backface culled (the faces of the model who's backs are towards the camera aren't rendered).
...
Text displayed about an object, empty by default for all objects except players, in which case, it is their name. The maximum number of characters is the `u16` max value.
...
When a player points at the object, they will be shown the text. The maximum number of characters is the `u16` max value.
...
Only when using "wielditem" visual. The maximum number of characters is the `u16` max value.
