...
=== `colors`
* field type: `ColorSpec table`

WARNING: This feature is not functional.
...
Whether a model is backface culled (the faces of the model who's back is towards you aren't shown)
...
Set the maximum automatic rotation to this value, no limit is the value is less than or equal to 0.
